...
class Mandate
{
	 ...
	 * @return Mandate
	 */
	protected function __construct(PDO $pdo,$idMandate,$test,$easyload=false)
	{
		// Sauvegarder pdo
		$this->pdo = $pdo;
	 ...
	 * @return Mandate
	 */
	public static function create(PDO $pdo,$test,$easyload=true)
	{
		// Ajouter le/la mandate dans la base de donn�es
		$pdoStatement = $pdo->prepare('INSERT INTO Mandate (test) VALUES (?)');
		if (!$pdoStatement->execute(array($test))) {
			throw new Exception('Erreur durant l\'insertion d\'un(e) mandate dans la base de donn�es');
		}

		// Construire le/la mandate
		return new Mandate($pdo,$pdo->lastInsertId(),$test,$easyload);
	}
	...
	{
		return $pdo->prepare('SELECT m.idMandate, m.test FROM Mandate m '.
	...
	public static function fetch(PDO $pdo,PDOStatement $pdoStatement,$easyload=false)
	{
		...
		list($idMandate,$test) = $values;

		// Construire le/la mandate
		return isset(Mandate::$easyload[$idMandate.'-'.$test]) ? Mandate::$easyload[$idMandate.'-'.$test] :
		new Mandate($pdo,$idMandate,$test,$easyload);
	}
	...
	{
		return $this->_set(array('test'),array($this->test));
	}
	 ...
	 * @return bool op�ration r�ussie ?
	 */
	public function setTest($test,$execute=true)
	{
		// Sauvegarder dans l'objet
		$this->test = $test;

		// Sauvegarder dans la base de donn�es (ou pas)
		return $execute ? $this->_set(array('test'),array($test)) : true;
	}
	...
	{
		return LiasonPasserelleMandat::selectByMandate($this->pdo,$this);
	}
	...
	{
		return LogTransfert::selectByMandate($this->pdo,$this);
	}
}
