...
class Passerelle
{
	 ...
	 * @return Passerelle
	 */
	protected function __construct(PDO $pdo,$idPasserelle,$name,$typeExport,$param,$easyload=false)
	{
		// Sauvegarder pdo
		$this->pdo = $pdo;
	 ...
	 * @return Passerelle
	 */
	public static function create(PDO $pdo,$name,$typeExport,$param,$easyload=true)
	{
		...
		if (!$pdoStatement->execute(array($name,$typeExport,$param))) {
			throw new Exception('Erreur durant l\'insertion d\'un(e) passerelle dans la base de donn�es');
		}

		// Construire le/la passerelle
		return new Passerelle($pdo,$pdo->lastInsertId(),$name,$typeExport,$param,$easyload);
	}
	...
	{
		return $pdo->prepare('SELECT p.idPasserelle, p.name, p.typeExport, p.param FROM Passerelle p '.
	...
	public static function fetch(PDO $pdo,PDOStatement $pdoStatement,$easyload=false)
	{
		...
		list($idPasserelle,$name,$typeExport,$param) = $values;

		// Construire le/la passerelle
		return isset(Passerelle::$easyload[$idPasserelle.'-'.$name.'-'.$typeExport.'-'.$param]) ? Passerelle::$easyload[$idPasserelle.'-'.$name.'-'.$typeExport.'-'.$param] :
		new Passerelle($pdo,$idPasserelle,$name,$typeExport,$param,$easyload);
	}
	...
	public function serialize($serialize=true)
	{
		...
		$array = array('idPasserelle' => $this->idPasserelle,'name' => $this->name,'typeExport' => $this->typeExport,'param' => $this->param);
	...
	public static function unserialize(PDO $pdo,$string,$easyload=true)
	{
		...
		new Passerelle($pdo,$array['idPasserelle'],$array['name'],$array['typeExport'],$array['param'],$easyload);
	}
	...
	{
		return $this->_set(array('name','typeExport','param'),array($this->name,$this->typeExport,$this->param));
	}
