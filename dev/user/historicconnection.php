...
class HistoricConnection
{
	 ...
	 */
	protected function __construct(PDO $pdo,$idHistoricConnection,$dateConnection,$user,$easyload=false)
	{
		// Sauvegarder pdo
		$this->pdo = $pdo;
	 ...
	 * @return HistoricConnection
	 */
	public static function create(PDO $pdo,$dateConnection,User $user,$easyload=true)
	{
		// Ajouter le/la historicConnection dans la base de donn�es
		$pdoStatement = $pdo->prepare('INSERT INTO HistoricConnection (dateConnection,user_idUser) VALUES (?,?)');
		if (!$pdoStatement->execute(array(date('Y-m-d H:i:s',$dateConnection),$user->getIdUser()))) {
	...
	{
		return $pdo->prepare('SELECT h.idHistoricConnection, h.dateConnection, h.user_idUser FROM HistoricConnection h '.
	...
	public function serialize($serialize=true)
	{
		...
		$array = array('idHistoricConnection' => $this->idHistoricConnection,'dateConnection' => $this->dateConnection,'user' => $this->user);
	...
	public static function unserialize(PDO $pdo,$string,$easyload=true)
	{
		...
		new HistoricConnection($pdo,$array['idHistoricConnection'],$array['dateConnection'],$array['user'],$easyload);
	}
	...
	{
		return $this->_set(array('dateConnection','user_idUser'),array(date('Y-m-d H:i:s',$this->dateConnection),$this->user));
	}
	...
	{
		$pdoStatement = $pdo->prepare('SELECT h.idHistoricConnection, h.dateConnection, h.user_idUser FROM HistoricConnection h WHERE h.user_idUser = ?');
