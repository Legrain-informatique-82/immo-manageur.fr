...
class User
{
	 ...
	 * @return User
	 */
	protected function __construct(PDO $pdo,$idUser,$identifiant,$password,$name,$firstname,$email,$registration_date,$levelMember,$agency,$easyload=false)
	{
		// Sauvegarder pdo
		$this->pdo = $pdo;
	 ...
	 * @return User
	 */
	public static function create(PDO $pdo,$identifiant,$password,$name,$firstname,$email,$registration_date,LevelMember $levelMember,Agency $agency,$easyload=true)
	{
		// Ajouter le/la user dans la base de donn�es
		$pdoStatement = $pdo->prepare('INSERT INTO User (identifiant,password,name,firstname,email,registration_date,levelMember_idLevelMember,agency_idAgency) VALUES (?,?,?,?,?,?,?,?)');
		if (!$pdoStatement->execute(array($identifiant,$password,$name,$firstname,$email,date('Y-m-d H:i:s',$registration_date),$levelMember->getIdLevelMember(),$agency->getIdAgency()))) {
			throw new Exception('Erreur durant l\'insertion d\'un(e) user dans la base de donn�es');
		}

		// Construire le/la user
		return new User($pdo,$pdo->lastInsertId(),$identifiant,$password,$name,$firstname,$email,$registration_date,$levelMember->getIdLevelMember(),$agency->getIdAgency(),$easyload);
	}
	...
	{
		return $pdo->prepare('SELECT u.idUser, u.identifiant, u.password, u.name, u.firstname, u.email, u.registration_date, u.levelMember_idLevelMember, u.agency_idAgency FROM User u '.
	...
	public static function fetch(PDO $pdo,PDOStatement $pdoStatement,$easyload=false)
	{
		...
		list($idUser,$identifiant,$password,$name,$firstname,$email,$registration_date,$levelMember,$agency) = $values;

		// Construire le/la user
		return isset(User::$easyload[$idUser.'-'.$identifiant.'-'.$password.'-'.$name.'-'.$firstname.'-'.$email.'-'.strtotime($registration_date).'-'.$levelMember.'-'.$agency]) ? User::$easyload[$idUser.'-'.$identifiant.'-'.$password.'-'.$name.'-'.$firstname.'-'.$email.'-'.strtotime($registration_date).'-'.$levelMember.'-'.$agency] :
		new User($pdo,$idUser,$identifiant,$password,$name,$firstname,$email,strtotime($registration_date),$levelMember,$agency,$easyload);
	}
	...
	public function serialize($serialize=true)
	{
		...
		$array = array('idUser' => $this->idUser,'identifiant' => $this->identifiant,'password' => $this->password,'name' => $this->name,'firstname' => $this->firstname,'email' => $this->email,'registration_date' => $this->registration_date,'levelMember' => $this->levelMember,'agency' => $this->agency);
	...
	public static function unserialize(PDO $pdo,$string,$easyload=true)
	{
		...
		new User($pdo,$array['idUser'],$array['identifiant'],$array['password'],$array['name'],$array['firstname'],$array['email'],$array['registration_date'],$array['levelMember'],$array['agency'],$easyload);
	}
	...
	{
		if (!($pdoStatement = $pdo->query('SELECT COUNT(idUser) FROM User'))) {
	...
	{
		return $this->_set(array('identifiant','password','name','firstname','email','registration_date','levelMember_idLevelMember','agency_idAgency'),array($this->identifiant,$this->password,$this->name,$this->firstname,$this->email,date('Y-m-d H:i:s',$this->registration_date),$this->levelMember,$this->agency));
	}
