...
class Seller
{
	 ...
	 * @return Seller
	 */
	protected function __construct(PDO $pdo,$idSeller,$name,$firstname,$address,$phone,$mobilPhone,$workPhone,$fax,$email,$comments,$numberUsed,$city,$sellerTitle,$user,$asset=true,$easyload=false)
	{
		// Sauvegarder pdo
		$this->pdo = $pdo;
	 ...
	 * @return Seller
	 */
	public static function create(PDO $pdo,$name,$firstname,$address,$phone,$mobilPhone,$workPhone,$fax,$email,$comments,$numberUsed,City $city,SellerTitle $sellerTitle,User $user,$asset=true,$easyload=true)
	{
		// Ajouter le/la seller dans la base de donn�es
		$pdoStatement = $pdo->prepare('INSERT INTO Seller (name,firstname,address,phone,mobilPhone,workPhone,fax,email,comments,numberUsed,city_idCity,sellerTitle_idSellerTitle,user_idUser,asset) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
		if (!$pdoStatement->execute(array($name,$firstname,$address,$phone,$mobilPhone,$workPhone,$fax,$email,$comments,$numberUsed,$city->getIdCity(),$sellerTitle->getIdSellerTitle(),$user->getIdUser(),$asset))) {
			throw new Exception('Erreur durant l\'insertion d\'un(e) seller dans la base de donn�es');
		}

		// Construire le/la seller
		return new Seller($pdo,$pdo->lastInsertId(),$name,$firstname,$address,$phone,$mobilPhone,$workPhone,$fax,$email,$comments,$numberUsed,$city->getIdCity(),$sellerTitle->getIdSellerTitle(),$user->getIdUser(),$asset,$easyload);
	}
	...
	{
		return $pdo->prepare('SELECT s.idSeller, s.name, s.firstname, s.address, s.phone, s.mobilPhone, s.workPhone, s.fax, s.email, s.comments, s.numberUsed, s.city_idCity, s.sellerTitle_idSellerTitle, s.user_idUser, s.asset FROM Seller s '.
	...
	public static function fetch(PDO $pdo,PDOStatement $pdoStatement,$easyload=false)
	{
		...
		list($idSeller,$name,$firstname,$address,$phone,$mobilPhone,$workPhone,$fax,$email,$comments,$numberUsed,$city,$sellerTitle,$user,$asset) = $values;

		// Construire le/la seller
		return isset(Seller::$easyload[$idSeller.'-'.$name.'-'.$firstname.'-'.$address.'-'.$phone.'-'.$mobilPhone.'-'.$workPhone.'-'.$fax.'-'.$email.'-'.$comments.'-'.$numberUsed.'-'.$city.'-'.$sellerTitle.'-'.$user.'-'.$asset]) ? Seller::$easyload[$idSeller.'-'.$name.'-'.$firstname.'-'.$address.'-'.$phone.'-'.$mobilPhone.'-'.$workPhone.'-'.$fax.'-'.$email.'-'.$comments.'-'.$numberUsed.'-'.$city.'-'.$sellerTitle.'-'.$user.'-'.$asset] :
		new Seller($pdo,$idSeller,$name,$firstname,$address,$phone,$mobilPhone,$workPhone,$fax,$email,$comments,$numberUsed,$city,$sellerTitle,$user,$asset,$easyload);
	}
	...
	public static function unserialize(PDO $pdo,$string,$easyload=true)
	{
		...
		new Seller($pdo,$array['idSeller'],$array['name'],$array['firstname'],$array['address'],$array['phone'],$array['mobilPhone'],$array['workPhone'],$array['fax'],$array['email'],$array['comments'],$array['numberUsed'],$array['city'],$array['sellerTitle'],$array['user'],$array['asset'],$easyload);
	}
	...
	{
		return $this->_set(array('name','firstname','address','phone','mobilPhone','workPhone','fax','email','comments','asset','numberUsed','city_idCity','sellerTitle_idSellerTitle','user_idUser'),array($this->name,$this->firstname,$this->address,$this->phone,$this->mobilPhone,$this->workPhone,$this->fax,$this->email,$this->comments,$this->asset,$this->numberUsed,$this->city,$this->sellerTitle,$this->user));
	}
	...
	{
		$pdoStatement = $pdo->prepare('SELECT s.idSeller, s.name, s.firstname, s.address, s.phone, s.mobilPhone, s.workPhone, s.fax, s.email, s.comments, s.numberUsed, s.city_idCity, s.sellerTitle_idSellerTitle, s.user_idUser, s.asset FROM Seller s WHERE s.city_idCity = ?');
