...
class Log
{
	...
	{
		$pdoStatement = $pdo->prepare('SELECT l.idLog, l.dateLog, l.pluginName, l.log, l.user_idUser FROM Log l WHERE l.user_idUser = ? '.$cond);
	 ...
	 * @return PDOStatement
	 */
	public static function selectByModule(PDO $pdo,$module,$cond='')
	{
		$pdoStatement = $pdo->prepare('SELECT l.idLog, l.dateLog, l.pluginName, l.log, l.user_idUser FROM Log l WHERE l.pluginName = ? '.$cond);
		if (!$pdoStatement->execute(array($module))) {
			throw new Exception('Erreur lors du chargement de tous/toutes les logs par user depuis la base de donn�es');
		}
		return $pdoStatement;
	}
	...
	public static function deleteByUser(PDO $pdo, User $user ){

		// Supprimer le/la log
		$pdoStatement = $pdo->prepare('DELETE FROM Log WHERE user_idUser = ?');
		if (!$pdoStatement->execute(array($user->getIdUser() ))) {
			throw new Exception('Erreur lors de la supression d\'un(e) log dans la base de donn�es');
		}
