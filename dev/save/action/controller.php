	...
	private function _treatment( $post,$get){
		...
		if(isset($get['page'])&&$get['page']=='add'){
			...
			if(isset($post['send'])){
				...
				elseif(!Tools::is_date_fr($initDate))$error[]='La date de début doit être au format suivant : jj/mm/YYYY.';
				if(!empty($deadDate)&&!Tools::is_date_fr($deadDate))$error[]='La date de fin doit être au format suivant : jj/mm/YYYY.';
