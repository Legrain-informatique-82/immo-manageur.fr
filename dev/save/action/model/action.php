...
class Action
{
	 ...
	 * @return Action
	 */
	public static function create(PDO $pdo,$libel,$initDate,$deadDate=null,$comment,User $from,User $to,$doDate=null,$mandate=null,$easyload=true)
	{
		...
		if (!$pdoStatement->execute(array($libel,date('Y-m-d H:i:s',$initDate),$deadDate==null?null:date('Y-m-d H:i:s',$deadDate),$comment,$from->getIdUser(),$to->getIdUser(),$doDate === null ? null : date('Y-m-d H:i:s',$doDate),$mandate == null ? null : $mandate->getIdMandate()))) {
			throw new Exception('Erreur durant l\'insertion d\'un(e) action dans la base de donn�es');
		}

		// Construire le/la action
		return new Action($pdo,$pdo->lastInsertId(),$libel,$initDate,$deadDate,$comment,$from->getIdUser(),$to->getIdUser(),$doDate,$mandate==null?null:$mandate->getIdMandate(),$easyload);
	}
	...
	{
		return $this->_set(array('libel','initDate','deadDate','comment','from_idUser','to_idUser','doDate','mandate_idMandate'),array($this->libel,date('Y-m-d H:i:s',$this->initDate),$this->deadDate == 0 ? null :date('Y-m-d H:i:s',$this->deadDate),$this->comment,$this->from,$this->to,$this->doDate == 0 ? null : date('Y-m-d H:i:s',$this->doDate),$this->mandate));
	}
	...
	public function setInitDate($initDate,$execute=true)
	{
		...
		// Sauvegarder dans la base de donn�es (ou pas)
		return $execute ? $this->_set(array('initDate'),array(date('Y-m-d H:i:s',$initDate))) : true;
	}
	 ...
	 * @return bool op�ration r�ussie ?
	 */
	public function setDeadDate($deadDate=null,$execute=true)
	{
		// Sauvegarder dans l'objet
		$this->deadDate = $deadDate;

		// Sauvegarder dans la base de donn�es (ou pas)
		return $execute ? $this->_set(array('deadDate'),array($deadDate === null ? null : date('Y-m-d H:i:s',$deadDate))) : true;
	}
	 ...
	 * @return bool op�ration r�ussie ?
	 */
	public function setTo(User $to,$execute=true)
	{
		// Sauvegarder dans l'objet
	...
	/**
	 * S�lectionner les actions par t
	 * @param $pdo PDO
	 * @param $to User
	 * @return PDOStatement
	 */
	public static function selectByTo(PDO $pdo,User $to,$dateButoir =null)
	{
		$sql = 'SELECT a.idAction, a.libel, a.initDate, a.deadDate, a.comment, a.from_idUser,a.to_idUser, a.doDate, a.mandate_idMandate FROM Action a WHERE a.to_idUser = ?';
		$array = array($to->getIdUser());
		if($dateButoir!=null){
			$sql.=' AND a.initDate <= ? ';
			$array[]=$dateButoir;
		}
		$sql.=' ORDER BY a.initDate ';
	...
	public function setDoDate($doDate=null,$execute=true)
	{
		...
		// Sauvegarder dans la base de donn�es (ou pas)
		return $execute ? $this->_set(array('doDate'),array($doDate === null ? null : date('Y-m-d H:i:s',$doDate))) : true;
	}
	 ...
	 * @return PDOStatement
	 */
	public static function selectByMandate(PDO $pdo,Mandate $mandate,$dateButoir =null)
	{
		$sql = 'SELECT a.idAction, a.libel, a.initDate, a.deadDate, a.comment, a.from_idUser,a.to_idUser, a.doDate, a.mandate_idMandate FROM Action a WHERE a.mandate_idMandate = ?';
		$array = array($mandate->getIdMandate());
		if($dateButoir){
			$sql.=' AND a.initDate <= ? ';
			$array[]=$dateButoir;
		}
	...
	{
		return '[Action idAction="'.$this->idAction.'" libel="'.$this->libel.'" initDate="'.date('d/m/Y',$this->initDate).'" deadDate="'.date('d/m/Y',$this->deadDate).'" comment="'.$this->comment.'" from="'.$this->from.'" doDate="'.date('d/m/Y H:i:s',$this->doDate).'" mandate="'.$this->mandate.'"]';
	}
