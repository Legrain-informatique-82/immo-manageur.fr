...
class MandateWaterCorresponding
{
	 ...
	 */
	protected function __construct(PDO $pdo,$idMandateWaterCorresponding,$name,$code,$easyload=false)
	{
		// Sauvegarder pdo
		$this->pdo = $pdo;
	...
	public static function create(PDO $pdo,$name,$code,$easyload=true)
	{
		...
		$pdoStatement = $pdo->prepare('INSERT INTO MandateWaterCorresponding (name,code) VALUES (?,?)');
		...
		return new MandateWaterCorresponding($pdo,$pdo->lastInsertId(),$name,$code,$easyload);
	}
	...
	{
		return $pdo->prepare('SELECT m.idMandateWaterCorresponding, m.name, m.code FROM MandateWaterCorresponding m '.
	...
	{
		$pdoStatement = MandateWaterCorresponding::_select($pdo);
	...
	public static function fetch(PDO $pdo,PDOStatement $pdoStatement,$easyload=false)
	{
		...
		list($idMandateWaterCorresponding,$name,$code) = $values;

		// Construire le/la mandateWaterCorresponding
		return isset(MandateWaterCorresponding::$easyload[$idMandateWaterCorresponding.'-'.$name.'-'.$code]) ? MandateWaterCorresponding::$easyload[$idMandateWaterCorresponding.'-'.$name.'-'.$code] :
		new MandateWaterCorresponding($pdo,$idMandateWaterCorresponding,$name,$code,$easyload);
	}
	...
	public function serialize($serialize=true)
	{
		...
		$array = array('idMandateWaterCorresponding' => $this->idMandateWaterCorresponding,'name' => $this->name,'code' => $this->code);
	...
	public static function unserialize(PDO $pdo,$string,$easyload=true)
	{
		...
		new MandateWaterCorresponding($pdo,$array['idMandateWaterCorresponding'],$array['name'],$array['code'],$easyload);
	}
