...
class MandateInsulation
{
	 ...
	 */
	protected function __construct(PDO $pdo,$idMandateInsulation,$name,$code,$easyload=false)
	{
		// Sauvegarder pdo
		$this->pdo = $pdo;
	...
	public static function create(PDO $pdo,$name,$code,$easyload=true)
	{
		...
		$pdoStatement = $pdo->prepare('INSERT INTO MandateInsulation (name,code) VALUES (?,?)');
		...
		return new MandateInsulation($pdo,$pdo->lastInsertId(),$name,$code,$easyload);
	}
	...
	{
		return $pdo->prepare('SELECT m.idMandateInsulation, m.name, m.code FROM MandateInsulation m '.
	...
	public static function loadAll(PDO $pdo,$easyload=false)
	{
		...
		$pdoStatement = MandateInsulation::selectAll($pdo);
	...
	{
		$pdoStatement = MandateInsulation::_select($pdo);
	...
	public static function fetch(PDO $pdo,PDOStatement $pdoStatement,$easyload=false)
	{
		...
		list($idMandateInsulation,$name,$code) = $values;

		// Construire le/la mandateInsulation
		return isset(MandateInsulation::$easyload[$idMandateInsulation.'-'.$name.'-'.$code]) ? MandateInsulation::$easyload[$idMandateInsulation.'-'.$name.'-'.$code] :
		new MandateInsulation($pdo,$idMandateInsulation,$name,$code,$easyload);
	}
	...
	public function serialize($serialize=true)
	{
		...
		$array = array('idMandateInsulation' => $this->idMandateInsulation,'name' => $this->name,'code' => $this->code);
	...
	public static function unserialize(PDO $pdo,$string,$easyload=true)
	{
		...
		new MandateInsulation($pdo,$array['idMandateInsulation'],$array['name'],$array['code'],$easyload);
	}
