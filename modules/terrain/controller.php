...
class Controller extends CoreController {
	...
	private function _treatment( $post,$get){
		...
		if(isset($get['page'])&&$get['page']=='preadd'){
			...
			$this->_title = 'Ajouter un terrain';
			...
			$this->_smarty->assign('typeTerrain',empty($post['typeTerrain'])?'1':$post['typeTerrain']);
			$this->_smarty->assign('situationTerrain',empty($post['situationTerrain'])?'1':$post['situationTerrain']);
			if(
			empty($listCity)||
			empty($listNotary)
			){
...
// 					elseif(strlen($post['numMandat'])>4) $error[] = Lang::ERROR_LENGHT_NUM_MANDAT;
					...
					if(empty($post['commissionMandat'])) $error[] =Lang::ERROR_EMPTY_COMMISSION;
					elseif(!is_numeric($post['commissionMandat']))$error[]=Lang::ERROR_FORMAT_TYPE_COMMISSION;
					if(empty($post['estimationFai'])) $error[] =Lang::ERROR_EMPTY_ESTIMATION_FAI;
					elseif(!is_numeric($post['estimationFai']))$error[]=Lang::ERROR_FORMAT_TYPE_ESTIMATION_FAI;
					if(empty($post['margeNegoce'])) $error[] =Lang::ERROR_EMPTY_MARGE_NEGOCE;
					elseif(!is_numeric($post['margeNegoce']))$error[]=Lang::ERROR_FORMAT_TYPE_MARGE_NEGOCE;
					...
					if(empty($error)){
						...
						$post['estimationFai'],$post['margeNegoce'],$post['refCadastre1'],$post['refCadastre2'],
						$post['refCadastre3'],$post['autreRefCadastre'],'','','','','','','',$post['numLot'],'',
							'','','','','','','','','','','','','','','','','','','','','','','',$user,$cityMandate->getSector(),
						$cityMandate,$notary,MandateType::load($this->_pdo,Constant::ID_PLOT_OF_LAND),
						TransactionType::load($this->_pdo,Constant::ID_TRANSACTION_TYPE_SELLER),MandateEtap::load($this->_pdo,Constant::ID_ETAP_TO_SELL),$post['libreMandat']==''?null:Tools::dateFrToTime($post['libreMandat'])
						);
						$mandat->setNature( $post['nature']==''?null:MandateNature::load($this->_pdo,$post['nature']));
						$mandat->setEstimationMaxi($post['estimationMaxi']);
						if($newSeller){
							Log::create($this->_pdo,time(),'seller','Creation du vendeur : '.$seller->getName(),$this->_user);
						}
						Log::create($this->_pdo,time(),'terrain','Ajout du terrain : '.$mandat->getNumberMandate(),$this->_user);
						...
						$mandat->setTypeTerrain($post['typeTerrain']);
						$mandat->setSituationTerrain($post['situationTerrain']);
						if($notaryAcq){
						...
						if(!empty($mandat)){
							...
							Log::create($this->_pdo,time(),'terrain','nouvelle liaison terrain/vendeur principal : '.$mandat->getNumberMandate().'/'.$seller->getName(),$this->_user);
						}
						/* Fin save */
						// Redirection
			...
			$this->_title = 'Liste des terrains';
			...
			// $post['agency'] contient all ou la valeur de l'agence.
			...
			$selectListMandat = Mandate::selectByEtapAndType($this->_pdo,$etap,MandateType::load($this->_pdo,Constant::ID_PLOT_OF_LAND));
			while($item = Mandate::fetch($this->_pdo,$selectListMandat) ){
				if($agency=='ALL' || $item->getUser()->getAgency()->getIdAgency()==$agency){
					$tmp['obj'] = $item;
					$tmp['urls']['see'] = $this->create_url($this->_user,'terrain','see',$item->getIdMandate());
					$tmp['urls']['duplicate'] = Tools::create_url($this->_user,'terrain','duplicate',$item->getIdMandate());
					$listMandat[] = $tmp;
				}
			}
			...
			$errorPicture = array();
			$this->_title = 'Fiche du terrain';
			$this->_template =dirname(__FILE__).'/views/see.tpl';
			// load du terrain
			$mandate = Mandate::load($this->_pdo,$get['action']);

			// Assign
			$this->_smarty->assign('premierMandat',Mandate::loadFirstOrLast( $this->_pdo,$mandate,0,1 ));
			$this->_smarty->assign('dernierMandat',Mandate::loadFirstOrLast( $this->_pdo,$mandate,1,1 ));
			$this->_smarty->assign('mandatPrecedent',Mandate::loadPreviewOrNext( $this->_pdo,$mandate,0,1 ));
			$this->_smarty->assign('mandatSuivant',Mandate::loadPreviewOrNext( $this->_pdo,$mandate,1 ,1));
				...
				if((($mandate->getUser()->getIdUser()!==$this->_user->getIdUser())&&$this->_user->getLevelMember()->getIdLevelMember()==3)&&($this->_user->getLevelMember()->getIdLevelMember()>2)){
					...
					if(isset($post['confirm'])&&$post['confirm']==1){
						...
						if(empty($error)){
							...
							$seller->setNumberUsed($seller->getNumberUsed()-1);
							Log::create($this->_pdo,time(),'terrain','Suppression de la liaison terrain/vendeur : '.$mandate->getNumberMandate().'/'.$nameOfSeller,$this->_user);
			...
			if(isset($post['sendSellerByDefault'])){
				if($this->_user->getLevelMember()->getIdLevelMember() < 3 || $this->_user->getIdUser() == $mandate->getUser()->getIdUser()){
					...
					Log::create($this->_pdo,time(),'terrain','Changement de vendeur principal pour le terrain '.$mandate->getNumberMandate(),$this->_user);
				}
				header('location:'.Tools::create_url($this->_user,$get['module'],'see',$post['idMandate']));
			}
			if(isset($post['sendPictureForMandate'])){
				if($this->_user->getLevelMember()->getIdLevelMember() < 3 || $this->_user->getIdUser() == $mandate->getUser()->getIdUser()){
					...
					if($file->goUpload()){
						...
						Log::create($this->_pdo,time(),'terrain','Ajout d\'une image pour le mandat : '.$mandate->getNumberMandate(),$this->_user);
			...
			if(isset($post['sendPlanForMandate'])){
				if($this->_user->getLevelMember()->getIdLevelMember() < 3 || $this->_user->getIdUser() == $mandate->getUser()->getIdUser()){
					if(!empty($_FILES['newPlan']['name'])){
						...
						Log::create($this->_pdo,time(),'terrain','Ajout d\'un plan pour le mandat : '.$mandate->getNumberMandate(),$this->_user);
				...
				if(isset($post['delete_plan'])&&$post['confirm']==1){
					if($this->_user->getLevelMember()->getIdLevelMember() < 3 || $this->_user->getIdUser() == $mandate->getUser()->getIdUser()){
						...
						if($pict->delete()){
							...
							Log::create($this->_pdo,time(),'terrain','Suppression du plan pour le terrain : '.$mandate->getNumberMandate(),$this->_user);
			...
			if(isset($post['delete_picture'])){
				if(isset($post['delete_picture'])&&$post['confirm']==1){
					if($this->_user->getLevelMember()->getIdLevelMember() < 3 || $this->_user->getIdUser() == $mandate->getUser()->getIdUser()){
						...
						if($pict->delete()){
							// suppression de la miniature et de l'image
							Log::create($this->_pdo,time(),'terrain','Suppression d\'une image pour le terrain : '.$mandate->getNumberMandate(),$this->_user);
							unlink(Constant::DEFAULT_PICTURE_MODULE_DIRECTORY.'/'.$get['module'].'/'.$nameOfPict);
							unlink(Constant::DEFAULT_PICTURE_MODULE_DIRECTORY.'/'.$get['module'].'/thumb/'.$nameOfPict);
						}
						header('location:'.Tools::create_url($this->_user,$get['module'],'see',$post['idMandate']));
			...
			if((($mandate->getUser()->getIdUser()!==$this->_user->getIdUser())&&$this->_user->getLevelMember()->getIdLevelMember()==3)&&($this->_user->getLevelMember()->getIdLevelMember()>2)){
				...
				if(isset($post['seller_add_submit_send'])){
					...
					if(empty($error)){
						...
						Log::create($this->_pdo,time(),'terrain','Affectation d\'un nouveau vendeur ('.$seller->getName().' ) au mandat : '.$mandate->getNumberMandate(),$this->_user);
				...
				if(isset($post['used'])){
					...
					Log::create($this->_pdo,time(),'terrain','Affectation d\'un nouveau vendeur ('.$newSeller->getName().' ) au mandat : '.$mandate->getNumberMandate(),$this->_user);
					header('location:'.Tools::create_url($this->_user,$get['module'],'see',$get['action']));
				}
			}
			...
			if((($mandate->getUser()->getIdUser()!==$this->_user->getIdUser())&&$this->_user->getLevelMember()->getIdLevelMember()==3)&&($this->_user->getLevelMember()->getIdLevelMember()>2)){
				...
				if(empty($post)){
					...
					$dateDeclPrealableDP =$mandate->getDateDeclarationPrealable()==null?'': date(Constant::DATE_FORMAT2,$mandate->getDateDeclarationPrealable());
				...
				if(isset($post['valider'])){
					...
					if(empty($error)){
						$mandate->setNouveaute( Tools::dateFrToTime($nouveauteSite),false);
						...
						$mandate->setGeolocalisation($latitude!=''&&$longitude!=''?$latitude.','.$longitude:'',false);
						...
						// manque les plans
						$mandate->update();
						Log::create($this->_pdo,time(),'terrain','Ajout/Modification des informations complementaires du terrain '.$mandate->getNumberMandate(),$this->_user);
						header('location:'.Tools::create_url($this->_user,$get['module'],'see',$get['action']));
		...
		elseif(isset($get['page'])&&$get['page']=='updateGen'){
			...
			if((($mandate->getUser()->getIdUser()!==$this->_user->getIdUser())&&$this->_user->getLevelMember()->getIdLevelMember()==3)&&($this->_user->getLevelMember()->getIdLevelMember()>2)){
				...
				if(empty($post)){
					...
					$typeTerrain = $mandate->getTypeTerrain();
					$situationTerrain = $mandate->getSituationTerrain();
					...
					$typeTerrain = $post['typeTerrain'];
					$situationTerrain = $post['situationTerrain'];
				}
				if(isset($post['cancel'])){
					...
					if($post['notary']){
						...
						$mandate->setTypeTerrain($typeTerrain,false);
						
						$mandate->setSituationTerrain($situationTerrain,false);
						...
						//$mandate->setNotary(Notary::load($this->_pdo,$notary),false);

						$mandate->update();
						Log::create($this->_pdo,time(),'terrain','Modification des infos générales du terrain '.$numMandat,$this->_user);
						header('location:'.Tools::create_url($this->_user,$get['module'],'see',$get['action']));
				...
				$this->_smarty->assign('typeTerrain',$typeTerrain);
				$this->_smarty->assign('situationTerrain',$situationTerrain);
			...
			if((($mandate->getUser()->getIdUser()!==$this->_user->getIdUser())&&$this->_user->getLevelMember()->getIdLevelMember()==3)&&($this->_user->getLevelMember()->getIdLevelMember()>2)){
				...
				if(empty($post)){
					...
					$permisAmenagerValide = $mandate->getPermisDamenagerValide();
					$terrainVendu = 0;
					if($mandate->getTerrainVenduViabilise()==1){
						$terrainVendu = 1;
					}elseif($mandate->getTerrainVenduSemiViabilise()==1){
						$terrainVendu = 2;
					}elseif($mandate->getTerrainVenduNonViabilise()==1){
						$terrainVendu = 3;
					}
					$passageEau = $mandate->getPassageEau();
					$passageElectricite = $mandate->getPassageElectricite();
					$passageGaz = $mandate->getPassageGaz();
					//$assainissement = $mandate->getToutALegout()?1:$mandate->getAssainissementParFosseSceptique()?2:0;
					$assainissement=0;
					if($mandate->getToutALegout()==1){
						$assainissement = 1;
					}elseif($mandate->getAssainissementParFosseSceptique()==1){
						$assainissement=2;
					}
					$voirie = $mandate->getVoirie();
					$tailleFacade = $mandate->getTailleFacade();
					...
					//$terrainSemiViabilise = $post['terrainSemiViabilise'];
					//$terrainNonViabilise = $post['terrainNonViabilise'];
					$terrainVendu = $post['terrainVendu'];
					...
					//$toutALEgout = $post['toutEgout'];
					//$assainissementParFosseSceptique = $post['assainissementFosseSceptique'];
					$assainissement = $post['assainissement'];
				...
				if(isset($post['cancel'])){
					...
					if($dateDeclPrealableDP!=''&&!Tools::is_date_fr($dateDeclPrealableDP))$error[]='La date de déclaration préalable doit être de la forme jj/mm/aaaa';
					if($dateProrogationDP!=''&&!Tools::is_date_fr($dateProrogationDP))$error[]='La date de prorogation préalable  doit être de la forme jj/mm/aaaa';
					if($dateCu!=''&&!Tools::is_date_fr($dateCu))$error[]='La date CU doit être de la forme jj/mm/aaaa';
					if($dateProrogationCu!=''&&!Tools::is_date_fr($dateProrogationCu))$error[]='La date de prorogation CU  doit être de la forme jj/mm/aaaa';
					if($dateDeclPrealableCUOPS!=''&&!Tools::is_date_fr($dateDeclPrealableCUOPS))$error[]='La date de CUOPS doit être de la forme jj/mm/aaaa';
					if($dateProrogationCUOPS!=''&&!Tools::is_date_fr($dateProrogationCUOPS))$error[]='La date de prorogation de CUOPS doit être de la forme jj/mm/aaaa';
					if($datePermisAmenager!=''&&!Tools::is_date_fr($datePermisAmenager))$error[]='La date du permis d\'amenager doit être de la forme jj/mm/aaaa';
					...
					if(empty($error)){
				 	...
				 	$mandate->setPassageElectricite($passageElectricite,false);
				 	$mandate->setPassageGaz($passageGaz,false);
				 	//$terrainVendu 0 = NC 1 = viabilisé 2 = semi viabilisé 3= non viabilisé
				 		
				 	switch ($terrainVendu){
				 		case 0:
				 			$mandate->setTerrainVenduViabilise(0,false);
				 			$mandate->setTerrainVenduSemiViabilise(0,false);
				 			$mandate->setTerrainVenduNonViabilise(0,false);
				 			break;
				 		case 1:
				 			$mandate->setTerrainVenduViabilise(1,false);
				 			$mandate->setTerrainVenduSemiViabilise(0,false);
				 			$mandate->setTerrainVenduNonViabilise(0,false);
				 			break;
				 		case 2:
				 			$mandate->setTerrainVenduViabilise(0,false);
				 			$mandate->setTerrainVenduSemiViabilise(1,false);
				 			$mandate->setTerrainVenduNonViabilise(0,false);
				 			break;
				 		case 3:
				 			$mandate->setTerrainVenduViabilise(0,false);
				 			$mandate->setTerrainVenduSemiViabilise(0,false);
				 			$mandate->setTerrainVenduNonViabilise(1,false);
				 			break;

				 	}
				 	// $assainissement 0 = NC pour les 2 - 1 = tout egout, 2 = fosse
				 	switch ($assainissement){
				 		case 0:
				 			$mandate->setToutALegout(0,false);
				 			$mandate->setAssainissementParFosseSceptique(0,false);
				 			break;
				 		case 1:
				 			$mandate->setToutALegout(1,false);
				 			$mandate->setAssainissementParFosseSceptique(0,false);
				 			break;
				 		case 2:
				 			$mandate->setToutALegout(0,false);
				 			$mandate->setAssainissementParFosseSceptique(2,false);
				 			break;
				 	}
				 	$mandate->setVoirie($voirie,false);
				 	$mandate->setTailleFacade($tailleFacade,false);
				 	...
				 	$mandate->setDateCU( $dateCu==''?null: Tools::dateFrToTime($dateCu),false);
				 	$mandate->setProrogationCUJusquau( $dateProrogationCu==''?null: Tools::dateFrToTime($dateProrogationCu),false);
				 	$mandate->setDateCuOPS( $dateDeclPrealableCUOPS==''?null: Tools::dateFrToTime($dateDeclPrealableCUOPS),false);
				 	$mandate->setProrogationCuOPSJusquau( $dateProrogationCUOPS==''?null: Tools::dateFrToTime($dateProrogationCUOPS),false);
				 	$mandate->setDatePermisDamenager( $datePermisAmenager==''?null: Tools::dateFrToTime($datePermisAmenager),false);
				 	$mandate->setProximiteEcole($proximiteEcole,false);
				 	$mandate->setProximiteCommerce($proximiteCommerce,false);
				 	...
				 	$mandate->setZonageRNU(MandateZonageRNU::load($this->_pdo,$idZonageRnu),false);
				 	$mandate->setCos(MandateCos::load($this->_pdo,$idCos),false);
				 	$mandate->setReferenceCadastreParcelle1($refCadastre1,false);
				 	$mandate->setReferenceCadastreParcelle2($refCadastre2,false);
				 	$mandate->setReferenceCadastreParcelle3($refCadastre3,false);
				 	$mandate->setAutreReferenceParcelle($autreRefCadastre,false);
				 	$mandate->setNumberLot($numLot,false);
				 	$mandate->update();
				 	Log::create($this->_pdo,time(),"terrain",'informations/descriptions modifiées/ajoutées',$this->_user );
				 	header('location:'.Tools::create_url($this->_user,$get['module'],'see',$get['action']));
				...
				$this->_smarty->assign('voirie',$voirie);
				$this->_smarty->assign('commentaire',$commentaire);
				$this->_smarty->assign('terrainVendu',$terrainVendu);
				$this->_smarty->assign('assainissement',$assainissement);
			...
			if((($mandate->getUser()->getIdUser()!==$this->_user->getIdUser())&&$this->_user->getLevelMember()->getIdLevelMember()==3)&&($this->_user->getLevelMember()->getIdLevelMember()>2)){
				...
				if(isset($post['valid'])){
					...
					$mandate->update();
					Log::create($this->_pdo,time(),'terrain','Modification des pub  du terrain '.$mandate->getNumberMandate() ,$this->_user);
			...
			if(empty($post)){
				...
				$finMandat = $mandate->getDeadDate()==''?'':date(Constant::DATE_FORMAT2,$mandate->getDeadDate());
			...
			if(isset($post['valider'])){
				...
				if(empty($error)){
					...
					if($update){
						$mandate->update();
						Log::create($this->_pdo,time(),'terrain','Modification des infos générale du mandat : '.$mandate->getNumberMandate(),$this->_user);
						header('location:'.Tools::create_url($this->_user,$get['module'],'see',$get['action']));
					}
				}
			}
			...
			if(empty($post)){
				...
				if(isset($post['valid'])){
					...
					Log::create( $this->_pdo,time(),'terrain','Annulation du mandat',$this->_user );
					header('location:'.Tools::create_url($this->_user,$get['module'],'see',$get['action']));
				}
				if(isset($post['cancel'])){
					header('location:'.Tools::create_url($this->_user,$get['module'],'see',$get['action']));
			...
			if(isset($post['cancel'])){
				header('location:'.Tools::create_url($this->_user,'terrain','see',$get['action']));
			}
			if(isset($post['valid'])){
				$error = array();
			...
			if(isset($post['cancel']))header('location:'.Tools::create_url($this->_user,'terrain','see',$mandate->getIdMandate()));
			if(isset($post['send'])){
				if(isset($post['unAcq'])){
					$acq->setActif(0);
				}
				...
				Log::create( $this->_pdo,time(),'terrain','finalisation de la vente du mandat',$this->_user );
				header('location:'.Tools::create_url($this->_user,'terrain','see',$mandate->getIdMandate()));
			...
			$a = Log::selectByModule($this->_pdo,'terrain');
			...
			if(empty($post)){
				...
				$typeTerrain = $m->getTypeTerrain();
				$situationTerrain = $m->getSituationTerrain();
			}else{
				$pubInternet = $post['pubInternet'];
				$nature= $post['nature'];
				$idUser = $post['idUser']?$post['idUser']:$m->getUser()->getIdUser();
				//$typeTransaction =$post['typeTransaction']?$post['typeTransaction']:$m->getTransactionType()->getIdTransactionType();
				$typeTransaction =$m->getTransactionType()->getIdTransactionType();
				$typeBien = $post['typeBien']?$post['typeBien']:$m->getMandateType()->getId();
			...
			if(isset($post['send'])){
				...
				if(empty($error)){
					...
					$newMandate->setTypeTerrain($typeTerrain);
					$newMandate->setSituationTerrain($situationTerrain);
