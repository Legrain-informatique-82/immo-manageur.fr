...
class MandateStyle
{
	 ...
	 * @return MandateStyle
	 */
	protected function __construct(PDO $pdo,$idMandateStyle,$name,$code,$isDisabled=0,$easyload=false)
	{
		// Sauvegarder pdo
		$this->pdo = $pdo;
	...
	public static function create(PDO $pdo,$name,$code,$isDisabled=0,$easyload=true)
	{
		...
		$pdoStatement = $pdo->prepare('INSERT INTO MandateStyle (name,code,isDisabled) VALUES (?,?,?)');
		...
		return new MandateStyle($pdo,$pdo->lastInsertId(),$name,$code,$isDisabled,$easyload);
	}
	...
	{
		return $pdo->prepare('SELECT m.idMandateStyle, m.name, m.code,m.isDisabled FROM MandateStyle m '.
	...
	public static function loadAll(PDO $pdo,$seeIsDisabled=0,$easyload=false)
	{
		...
		if($seeIsDisabled){
			$pdoStatement = MandateStyle::selectAll($pdo,null,'m.name');
		}else{
			$pdoStatement = MandateStyle::selectAll($pdo,'m.isDisabled = 0','m.name');
		}
	...
	{
		$pdoStatement = MandateStyle::_select($pdo,$where,$orderby);
	...
	public static function fetch(PDO $pdo,PDOStatement $pdoStatement,$easyload=false)
	{
		...
		list($idMandateStyle,$name,$code,$isDisabled) = $values;

		// Construire le/la mandateStyle
		return isset(MandateStyle::$easyload[$idMandateStyle.'-'.$name.'-'.$code.'-'.$isDisabled]) ? MandateStyle::$easyload[$idMandateStyle.'-'.$name.'-'.$code.'-'.$isDisabled] :
		new MandateStyle($pdo,$idMandateStyle,$name,$code,$isDisabled,$easyload);
	}
	...
	public function serialize($serialize=true)
	{
		...
		$array = array('idMandateStyle' => $this->idMandateStyle,'name' => $this->name,'code' => $this->code,'isDisabled'=> $isDisabled);
	...
	public static function unserialize(PDO $pdo,$string,$easyload=true)
	{
		...
		new MandateStyle($pdo,$array['idMandateStyle'],$array['name'],$array['code'],$array['isDisabled'],$easyload);
	}
	...
	{
		$pdoStatement = $pdo->prepare('SELECT COUNT(idMandateStyle) FROM MandateStyle WHERE name = ?');
		if (!($pdoStatement->execute( array($name)) )) {
			throw new Exception('Erreur lors du comptage des MandateStyle dans la base de donn�es');
		}
		return($pdoStatement->fetchColumn());
	...
	{
		$pdoStatement = $pdo->prepare('SELECT COUNT(idMandateStyle) FROM MandateStyle WHERE code = ?');
