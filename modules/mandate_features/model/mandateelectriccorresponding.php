...
class MandateElectricCorresponding
{
	 ...
	 */
	protected function __construct(PDO $pdo,$idMandateElectricCorresponding,$name,$code,$isDisabled,$easyload=false)
	{
		// Sauvegarder pdo
		$this->pdo = $pdo;
	...
	public static function create(PDO $pdo,$name,$code,$isDisabled=0,$easyload=true)
	{
		...
		$pdoStatement = $pdo->prepare('INSERT INTO MandateElectricCorresponding (name,code,isDisabled) VALUES (?,?,?)');
		...
		return new MandateElectricCorresponding($pdo,$pdo->lastInsertId(),$name,$code,$isDisabled,$easyload);
	}
	...
	{
		return $pdo->prepare('SELECT m.idMandateElectricCorresponding, m.name, m.code,m.isDisabled FROM MandateElectricCorresponding m '.
	...
	public static function loadAll(PDO $pdo,$seeIsDisabled=0,$easyload=false)
	{
		...
		if($seeIsDisabled){
			...
			$pdoStatement = MandateElectricCorresponding::selectAll($pdo,'m.isDisabled = 0');
		}
	...
	{
		$pdoStatement = MandateElectricCorresponding::_select($pdo,$where,$orderby);
	...
	public static function fetch(PDO $pdo,PDOStatement $pdoStatement,$easyload=false)
	{
		...
		list($idMandateElectricCorresponding,$name,$code,$isDisabled) = $values;

		// Construire le/la mandateElectricCorresponding
		return isset(MandateElectricCorresponding::$easyload[$idMandateElectricCorresponding.'-'.$name.'-'.$code.'-'.$isDisabled]) ? MandateElectricCorresponding::$easyload[$idMandateElectricCorresponding.'-'.$name.'-'.$code.'-'.$isDisabled] :
		new MandateElectricCorresponding($pdo,$idMandateElectricCorresponding,$name,$code,$isDisabled,$easyload);
	}
	...
	public function serialize($serialize=true)
	{
		...
		$array = array('idMandateElectricCorresponding' => $this->idMandateElectricCorresponding,'name' => $this->name,'code' => $this->code,'isDisabled'=>$this->isDisabled );
	...
	public static function unserialize(PDO $pdo,$string,$easyload=true)
	{
		...
		new MandateElectricCorresponding($pdo,$array['idMandateElectricCorresponding'],$array['name'],$array['code'],$array['isDisabled'],$easyload);
	}
	...
	{
		$pdoStatement = $pdo->prepare('SELECT COUNT(idMandateElectricCorresponding) FROM MandateElectricCorresponding WHERE name = ?');
		if (!($pdoStatement->execute( array($name)) )) {
			throw new Exception('Erreur lors du comptage des MandateElectricCorresponding dans la base de donn�es');
		}
		return($pdoStatement->fetchColumn());
	...
	{
		$pdoStatement = $pdo->prepare('SELECT COUNT(idMandateElectricCorresponding) FROM MandateElectricCorresponding WHERE code = ?');
