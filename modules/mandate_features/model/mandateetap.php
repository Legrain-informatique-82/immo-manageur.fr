...
const FIELDNAME_IDMANDATEETAP='idMandateEtap';
	 ...
	 * @return MandateEtap
	 */
	protected function __construct(PDO $pdo,$idMandateEtap,$name,$code,$isDisabled,$easyload=false)
	{
		// Sauvegarder pdo
		$this->pdo = $pdo;
	...
	public static function create(PDO $pdo,$name,$code,$easyload=true)
	{
		...
		$pdoStatement = $pdo->prepare('INSERT INTO MandateEtap (name,code,isDisabled) VALUES (?,?,?)');
		...
		return new MandateEtap($pdo,$pdo->lastInsertId(),$name,$code,$isDisabled,$easyload);
	}
	...
	{
		return $pdo->prepare('SELECT m.idMandateEtap, m.name, m.code,m.isDisabled FROM MandateEtap m '.
	...
	public static function loadAll(PDO $pdo,$seeIsDisabled=0,$easyload=false)
	{
		...
		if($seeIsDisabled){
			...
			$pdoStatement = MandateEtap::selectAll($pdo,'m.isDisabled = 0');
		}
	...
	{
		$pdoStatement = MandateEtap::_select($pdo,$where,$orderby);
	...
	public static function fetch(PDO $pdo,PDOStatement $pdoStatement,$easyload=false)
	{
		...
		// Construire le/la mandateEtap
		return isset(MandateEtap::$easyload[$idMandateEtap.'-'.$name.'-'.$code.'-'.$isDisabled]) ? MandateEtap::$easyload[$idMandateEtap.'-'.$name.'-'.$code.'-'.$isDisabled] :
		new MandateEtap($pdo,$idMandateEtap,$name,$code,$isDisabled,$easyload);
	}
	...
	public function serialize($serialize=true)
	{
		...
		$array = array('idMandateEtap' => $this->idMandateEtap,'name' => $this->name,'code' => $this->code,'isDisabled'=> $this->isDisabled);
	...
	public static function unserialize(PDO $pdo,$string,$easyload=true)
	{
		...
		new MandateEtap($pdo,$array['idMandateEtap'],$array['name'],$array['code'],$array['isDisabled'],$easyload);
	}
