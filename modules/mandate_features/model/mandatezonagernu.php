...
class MandateZonageRNU
{
	 ...
	 */
	protected function __construct(PDO $pdo,$idMandateZonageRNU,$name,$code,$isDisabled ,$easyload=false)
	{
		// Sauvegarder pdo
		$this->pdo = $pdo;
	...
	public static function create(PDO $pdo,$name,$code,$isDisabled=0,$easyload=true)
	{
		...
		$pdoStatement = $pdo->prepare('INSERT INTO MandateZonageRNU (name,code,isDisabled) VALUES (?,?,?)');
		...
		return new MandateZonageRNU($pdo,$pdo->lastInsertId(),$name,$code,$isDisabled,$easyload);
	}
	...
	{
		return $pdo->prepare('SELECT m.idMandateZonageRNU, m.name, m.code, m.isDisabled FROM MandateZonageRNU m '.
	...
	public static function loadAll(PDO $pdo,$seeIsDisabled=0,$easyload=false)
	{
		...
		if($seeIsDisabled){
			...
			$pdoStatement = MandateZonageRNU::selectAll($pdo,'m.isDisabled = 0');
		}
	...
	{
		$pdoStatement = MandateZonageRNU::_select($pdo,$where,$orderby);
	...
	public static function fetch(PDO $pdo,PDOStatement $pdoStatement,$easyload=false)
	{
		...
		list($idMandateZonageRNU,$name,$code,$isDisabled) = $values;

		// Construire le/la mandateZonageRNU
		return isset(MandateZonageRNU::$easyload[$idMandateZonageRNU.'-'.$name.'-'.$code.'-'.$isDisabled]) ? MandateZonageRNU::$easyload[$idMandateZonageRNU.'-'.$name.'-'.$code.'-'.$isDisabled] :
		new MandateZonageRNU($pdo,$idMandateZonageRNU,$name,$code,$isDisabled,$easyload);
	}
	...
	public function serialize($serialize=true)
	{
		...
		$array = array('idMandateZonageRNU' => $this->idMandateZonageRNU,'name' => $this->name,'code' => $this->code,'isDisabled'=> $this->isDisabled);
	...
	public static function unserialize(PDO $pdo,$string,$easyload=true)
	{
		...
		new MandateZonageRNU($pdo,$array['idMandateZonageRNU'],$array['name'],$array['code'],$array['isDisabled'],$easyload);
	}
	...
	{
		$pdoStatement = $pdo->prepare('SELECT COUNT(idMandateZonageRNU) FROM MandateZonageRNU WHERE name = ?');
		if (!($pdoStatement->execute( array($name)) )) {
			throw new Exception('Erreur lors du comptage des MandateZonageRNU dans la base de donn�es');
		}
		return($pdoStatement->fetchColumn());
	...
	{
		$pdoStatement = $pdo->prepare('SELECT COUNT(idMandateZonageRNU) FROM MandateZonageRNU WHERE code = ?');
