...
class MandatePicture
{
	...
	{
		$pdoStatement = $pdo->prepare('SELECT m.idMandatePicture, m.name, m.isDefault, m.fk_idMandate FROM MandatePicture m WHERE m.fk_idMandate = ? ORDER BY m.isDefault DESC');
	...
	* @return PDOStatement
	*/
	public static function selectByMandateAndDefault(PDO $pdo,Mandate $mandate)
	{
		$pdoStatement = $pdo->prepare('SELECT m.idMandatePicture, m.name, m.isDefault, m.fk_idMandate FROM MandatePicture m WHERE m.fk_idMandate = ? AND m.isDefault = 1');
		if (!$pdoStatement->execute(array($mandate->getIdMandate()))) {
			throw new Exception('Erreur lors du chargement de tous/toutes les mandatePictures par mandate depuis la base de donn�es');
		}
		return $pdoStatement;
