...
if(!class_exists( Export_poliris_with_pictures_in_url )){
	class Export_poliris_with_pictures_in_url implements modelExport {
		...
		public function __construct(PDO$pdo, Passerelle $passerelle) {
			...
			if(!is_dir(Constant::DEFAULT_DIR_EXPORT_POLIRIS_WITH_URL)){
				mkdir(Constant::DEFAULT_DIR_EXPORT_POLIRIS_WITH_URL, 0777);
				chmod(Constant::DEFAULT_DIR_EXPORT_POLIRIS_WITH_URL, 0777);
			}
			
			// Suppression du répertoire contenant les photos de l'ancienne exportation.
			if(is_dir(Constant::DEFAULT_DIR_EXPORT_POLIRIS_WITH_URL.$this -> _param['codeAgence'].'/')){
				foreach( glob(Constant::DEFAULT_DIR_EXPORT_POLIRIS_WITH_URL.$this -> _param['codeAgence'].'/*') as $fi){
					if(is_file($fi)){
						@unlink($fi);
					}
				}
				@rmdir( Constant::DEFAULT_DIR_EXPORT_POLIRIS_WITH_URL.$this -> _param['codeAgence'] );
				
			}
			// Creation du répertoire contenant les photos
			mkdir(Constant::DEFAULT_DIR_EXPORT_POLIRIS_WITH_URL.$this -> _param['codeAgence'],0777);
			chmod(Constant::DEFAULT_DIR_EXPORT_POLIRIS_WITH_URL.$this -> _param['codeAgence'], 0777);
...
// 								
								...
								// Ajout des photos dans le répertoire contenant les photos pour le service donné et accèssible via URL.
								copy(
								Constant::DEFAULT_PICTURE_MODULE_DIRECTORY.$module.'/'.$pict->getName(),
								
								Constant::DEFAULT_DIR_EXPORT_POLIRIS_WITH_URL.$this -> _param['codeAgence'].'/'.$codeAgence.'-'.$m->getIdMandate().'_'.$i.'.jpg'
								);
								
								//$zip->addFile($this->_tmpFolder.$codeAgence.'-'.$m->getIdMandate().'_'.$i.'.jpg',$codeAgence.'-'.$m->getIdMandate().'_'.$i.'.jpg');
								
								// champs des photos
								$line[$comptePicture]=Constant::DEFAULT_URL_EXPORT_POLIRIS_WITH_URL.$this -> _param['codeAgence'].'/'.$codeAgence.'-'.$m->getIdMandate().'_'.$i.'.jpg';
...
$objExport = new Export_poliris_with_pictures_in_url($pdo, $passerelle);
//unset($objExport);
