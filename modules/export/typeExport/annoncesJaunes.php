...
if(!class_exists(Export_annonces_jaunes)) {
	class Export_annonces_jaunes implements modelExport {
		...
		public function __construct(PDO$pdo, Passerelle$passerelle) {
			...
			$this -> _tmpFolder = Constant::DEFAULT_TMP_DIRECTORY . 'export_annonces_jaunes_'.$this->_dateFichier.'/';
		...
		private function sendOnFtp() {
			...
			*/
			...
			if(!is_dir( Constant::DEFAULT_LOGS_DIRECTORY.'exports/'.date('Ymd').'/export_annonces_jaunes_'.$this->_dateFichier) ){
				mkdir(Constant::DEFAULT_LOGS_DIRECTORY.'exports/'.date('Ymd').'/export_annonces_jaunes_'.$this->_dateFichier);
				chmod(Constant::DEFAULT_LOGS_DIRECTORY.'exports/'.date('Ymd').'/export_annonces_jaunes_'.$this->_dateFichier, 0777);
			}
			copy($this -> _tmpFolder.'log.txt', Constant::DEFAULT_LOGS_DIRECTORY.'exports/'.date('Ymd').'/export_annonces_jaunes_'.$this->_dateFichier.'/log.txt');
			copy($this -> _tmpFolder . 'annoncesJaunes.zip', Constant::DEFAULT_LOGS_DIRECTORY.'exports/'.date('Ymd').'/export_annonces_jaunes_'.$this->_dateFichier.'/annoncesJaunes.zip');
