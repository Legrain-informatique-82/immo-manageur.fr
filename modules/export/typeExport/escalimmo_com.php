...
if(!class_exists( Export_escalimmo_com )){
	class Export_escalimmo_com implements modelExport{
		...
		public function __construct(PDO $pdo, Passerelle $passerelle){
			...
			$this->_tmpFolder = Constant::DEFAULT_TMP_DIRECTORY.'export_escalimmo_com_'.$this->_dateFichier.'/';
		...
		private function sendOnFtp(){
			...
			if(!is_dir( Constant::DEFAULT_LOGS_DIRECTORY.'exports/'.date('Ymd').'/export_escalimmo_com_'.$this->_dateFichier) ){
				mkdir(Constant::DEFAULT_LOGS_DIRECTORY.'exports/'.date('Ymd').'/export_escalimmo_com_'.$this->_dateFichier);
				chmod(Constant::DEFAULT_LOGS_DIRECTORY.'exports/'.date('Ymd').'/export_escalimmo_com_'.$this->_dateFichier, 0777);
			}
			...
			$ftp->connexion();
			$ftp->cd('Perso');
			$this->setLog(date('Y-m-d H:i:s')." - Début du transfert des bases \n");
			...
			foreach( glob($this->_tmpFolder.'base/*') as $fileToUpload){
				...
				$this->setLog(date('Y-m-d H:i:s')." - tentative de transfert de ".$fileToUpload." (".filesize($fileToUpload).") \n");
				if($ftp->upload($name,$fileToUpload,FTP_BINARY)){
					$this->setLog(date('Y-m-d H:i:s')." - transfert de ".$fileToUpload." terminé \n");

				}else{
					$this->setLog(date('Y-m-d H:i:s')." - Echec du transfert de ".$fileToUpload." \n");
				}
			}
			$this->setLog(date('Y-m-d H:i:s')." - Fin du transfert des bases \n");
			// idem pour les images
			$ftp->cd('../Photos');
			$this->setLog(date('Y-m-d H:i:s')." - Début du transfert des Photos \n");
			foreach($ftp->ls() as $file){
			...
			foreach( glob($this->_tmpFolder.'pictures/*') as $fileToUpload){
				...
				if($ftp->upload($name,$fileToUpload,FTP_BINARY)){
					...
					$this->setLog(date('Y-m-d H:i:s')." - Echec du transfert de ".$fileToUpload." \n");
					mail('julien@legrain.fr', 'escal82.com', 'Erreur lors d\'un export ( escalimmo)');
				}
			}
			$this->setLog(date('Y-m-d H:i:s')." - Fin du transfert des Photos \n");
			unset($ftp);
			
		
			copy($this -> _tmpFolder.'log.txt', Constant::DEFAULT_LOGS_DIRECTORY.'exports/'.date('Ymd').'/export_escalimmo_com_'.$this->_dateFichier.'/log.txt');
			// copie des bases et des photos
			Tools::CopieRep($this -> _tmpFolder.'base', Constant::DEFAULT_LOGS_DIRECTORY.'exports/'.date('Ymd').'/export_escalimmo_com_'.$this->_dateFichier.'/base');
			Tools::CopieRep($this -> _tmpFolder.'pictures', Constant::DEFAULT_LOGS_DIRECTORY.'exports/'.date('Ymd').'/export_escalimmo_com_'.$this->_dateFichier.'/pictures');
