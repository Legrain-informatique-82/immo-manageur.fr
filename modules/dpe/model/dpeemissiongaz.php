...
class DpeEmissionGaz
{
	...
	{
		return $pdo->prepare('SELECT d.idDpeEmissionGaz, d.name, d.fromD, d.toD, d.position FROM DpeEmissionGaz d '.
	...
	public static function loadByValue(PDO $pdo,$value,$easyload=true)
	{


		// Charger le/la dpeConsoEner
		$pdoStatement = DpeEmissionGaz::_select($pdo,'d.fromD <= ? AND d.toD >= ? ');

		if (!$pdoStatement->execute(array($value,$value))) {
