...
class fpdf_multicelltag extends FPDF{
	public $txtFooter =  null;
	public $baniere=null;
	public $logoFooter=null;
	public $useEntete =false;
	public $useFooter =false;
	public $useHeader = true;

	function header(){

		if($this->useHeader){
// 			var_dump('test'.$this->useHeader);
			if($this->useEntete){
				$this->SetTopMargin(0);
				$this->SetFont('times','',12);
					
				// mettre une variable
				if(is_file( Constant::DEFAULT_PICTURE_MODULE_DIRECTORY.'user/'.$this->baniere ))
				$this->Image(Constant::DEFAULT_PICTURE_MODULE_DIRECTORY.'user/'.$this->baniere,5,5);
				if($this->PageNo() <= 1){
					$this->ln(40);
				}else{
					$this->ln(90);
	...
	function footer(){
		if($this->useFooter){
			...
			if(is_file( Constant::DEFAULT_PICTURE_MODULE_DIRECTORY.'user/'.$this->logoFooter ))
			$this->Image(Constant::DEFAULT_PICTURE_MODULE_DIRECTORY.'user/'.$this->logoFooter,5,$this->GetY());
			// 			$txt = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed imperdiet tristique est mattis egestas. Vivamus pretium mauris ac urna dictum facilisis vel in ligula. Praesent eleifend lorem ut quam convallis non pulvinar mi bibendum. Nunc magna quam, rutrum in vulputate sit amet, blandit et ligula. In commodo sodales massa, sit amet commodo mauris pharetra non. Etiam et tincidunt augue. Sed ac tortor quis odio bibendum ornare eget quis orci. Donec euismod malesuada velit, a fringilla nisi suscipit at. Sed pulvinar felis et metus ultricies accumsan. Cras velit est, pellentesque sit amet dignissim eget, vulputate nec orci.';

			if ($this->txtFooter){
				$this->MultiCell(0, 3, utf8_decode( Tools::replaceSpecialCharByIso($this->txtFooter )) );
			}
		}
	}
	/**
		...
		foreach ($aDataInfo as $key => $val){
			...
			while ($i < $s_lenght){
				...
				if(is_int(strpos(" ,.:;",$c))){
					...
					$last_sepch = $c;//last separatorm char
...
class fpdf_multicelltagBis extends FPDF{
// 	public $txtFooter =  null;
// 	public $baniere=null;
// 	public $logoFooter=null;
// 	public $useEntete =false;
...
class Ffpdf_multicelltag extends fpdf_multicelltag{
	function Footer(){
		$this->SetFont('times','',8);
		$this->setY(-15);
		$this->Cell(0,10,utf8_decode( 'Document non contractuel ne pouvant engager la responsabilité de l\'agence' ),0,0,'C');
