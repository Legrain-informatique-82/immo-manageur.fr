...
class User
{
	 ...
	 * @param $theme String du theme utilisé
	 * @param $openInNewTab Bool Ouvre les liens dans une nouvelle table
	 * @param $easyload bool activer le chargement rapide ?
	 * @return User
	 */
	protected function __construct(PDO $pdo,$idUser,$identifiant,$password,$name,$firstname,$email,$registration_date,$numberUsed,$levelMember,$agency,$theme,$openInNewTab,$cellPhone,$easyload=false)
	{
		// Sauvegarder pdo
		$this->pdo = $pdo;
	 ...
	 * @return User
	 */
	public static function create(PDO $pdo,$identifiant,$password,$name,$firstname,$email,$registration_date,$numberUsed,LevelMember $levelMember,Agency $agency,$theme='',$openInNewTab,$cellPhone,$easyload=true)
	{
		// Ajouter le/la user dans la base de donn�es
		$pdoStatement = $pdo->prepare('INSERT INTO User (identifiant,password,name,firstname,email,registration_date,numberUsed,levelMember_idLevelMember,agency_idAgency,theme,openInNewTab,cellPhone) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)');
		if (!$pdoStatement->execute(array($identifiant,$password,$name,$firstname,$email,date('Y-m-d H:i:s',$registration_date),$numberUsed,$levelMember->getIdLevelMember(),$agency->getIdAgency(),$theme,$openInNewTab,$cellPhone))) {
			throw new Exception('Erreur durant l\'insertion d\'un(e) user dans la base de donn�es');
		}

		// Construire le/la user
		return new User($pdo,$pdo->lastInsertId(),$identifiant,$password,$name,$firstname,$email,$registration_date,$numberUsed,$levelMember->getIdLevelMember(),$agency->getIdAgency(),$theme,$openInNewTab,$cellPhone,$easyload);
	}
	...
	{
        return $pdo->prepare('SELECT u.idUser, u.identifiant, u.password, u.name, u.firstname, u.email, u.registration_date, u.numberUsed ,u.levelMember_idLevelMember, u.agency_idAgency, u.theme,u.openInNewTab,u.cellPhone FROM User u '.
	...
	public static function loadAll(PDO $pdo,$easyload=false)
	{
		...
		$pdoStatement = User::selectAll($pdo,null,'u.firstname , u.name ');
	 ...
	 * @return PDOStatement
	 */
	public static function selectAll(PDO $pdo,$where=null,$order=null)
	{
		$pdoStatement = User::_select($pdo,$where,$order);
	...
	public static function fetch(PDO $pdo,PDOStatement $pdoStatement,$easyload=false)
	{
		...
		list($idUser,$identifiant,$password,$name,$firstname,$email,$registration_date,$numberUsed,$levelMember,$agency,$theme,$openInNewTab,$cellPhone) = $values;

		// Construire le/la user
		return isset(User::$easyload[$idUser.'-'.$identifiant.'-'.$password.'-'.$name.'-'.$firstname.'-'.$email.'-'.strtotime($registration_date).'-'.$numberUsed.'-'.$levelMember.'-'.$agency.'-'.$theme.'-'.$openInNewTab.'-'.$cellPhone]) ? User::$easyload[$idUser.'-'.$identifiant.'-'.$password.'-'.$name.'-'.$firstname.'-'.$email.'-'.strtotime($registration_date).'-'.$numberUsed.'-'.$levelMember.'-'.$agency.'-'.$theme.'-'.$openInNewTab.'-'.$cellPhone] :
		new User($pdo,$idUser,$identifiant,$password,$name,$firstname,$email,strtotime($registration_date),$numberUsed,$levelMember,$agency,$theme,$openInNewTab,$cellPhone,$easyload);
	}
	...
	public function serialize($serialize=true)
	{
		...
		$array = array('idUser' => $this->idUser,'identifiant' => $this->identifiant,'password' => $this->password,'name' => $this->name,'firstname' => $this->firstname,'email' => $this->email,'registration_date' => $this->registration_date,'numberUsed' => $this->numberUsed,'levelMember' => $this->levelMember,'agency' => $this->agency,'theme' => $this->theme, 'openInNewTab' => $this->openInNewTab,'cellPhone'=> $this->cellPhone);
	...
	public static function unserialize(PDO $pdo,$string,$easyload=true)
	{
		...
		new User($pdo,$array['idUser'],$array['identifiant'],$array['password'],$array['name'],$array['firstname'],$array['email'],$array['registration_date'],$array['numberUsed'],$array['levelMember'],$array['agency'],$array['theme'],$array['openInNewTab'],$array['cellPhone'],$easyload);
	}
	...
	{
		return $this->_set(array('identifiant','password','name','firstname','email','registration_date','numberUsed','levelMember_idLevelMember','agency_idAgency','theme','openInNewTab','cellPhone'),array($this->identifiant,$this->password,$this->name,$this->firstname,$this->email,date('Y-m-d H:i:s',$this->registration_date),$this->numberUsed,$this->levelMember,$this->agency,$this->theme,$this->openInNewTab,$this->cellPhone));
	}
	...
	{
		$pdoStatement = $pdo->prepare('SELECT u.idUser, u.identifiant, u.password, u.name, u.firstname, u.email, u.registration_date, u.levelMember_idLevelMember, u.agency_idAgency, u.theme,u.openInNewTab,u.cellPhone FROM User u WHERE u.levelMember_idLevelMember = ?');
	...
	{
		$pdoStatement = $pdo->prepare('SELECT u.idUser, u.identifiant, u.password, u.name, u.firstname, u.email, u.registration_date, u.levelMember_idLevelMember, u.agency_idAgency, u.theme,u.openInNewTab,u.cellPhone FROM User u WHERE u.agency_idAgency = ?');
	...
	/**
		...
		* @return bool op�ration r�ussie ?
		*/
	public function setTheme( $theme,$execute=true)
	{
		// Sauvegarder dans l'objet
		$this->theme = $theme;

		// Sauvegarder dans la base de donn�es (ou pas)
		return $execute ? $this->_set(array('theme'),array($theme)) : true;
	}
	...
	/**
	 * D�finir le/la openInNewTab
	 * @param $openInNewTab String
	 * @param $execute bool ex�cuter la requ�te update ?
	 * @return bool op�ration r�ussie ?
	 */
	public function setOpenInNewTab( $openInNewTab,$execute=true)
	{
		// Sauvegarder dans l'objet
		$this->openInNewTab = $openInNewTab;

		// Sauvegarder dans la base de donn�es (ou pas)
		return $execute ? $this->_set(array('openInNewTab'),array($openInNewTab)) : true;
	}
