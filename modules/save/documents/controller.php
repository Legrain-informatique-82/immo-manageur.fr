...
class Controller extends CoreController {
	...
	private function _treatment( $post,$get){
		if(isset($get['page'])&&$get['page']=='bonVisite'){
			...
			$initDate =  Tools::dateEnToTime('1970-09-01');
			...
			if(empty($mandats)){
				$fpdf->AddPage();
				if(strtoupper( $acq->getUser()->getAgency()->getName())=='MONTAUBAN'){
					$fpdf->Image( Constant::DEFAULT_URL_PICTURE_DIRECTORY.'logoEscalTerrains.jpg' ,null,null,0,0,'jpg' );
				}else{
					$fpdf->Image( Constant::DEFAULT_URL_PICTURE_DIRECTORY.'logoEscalImmo.jpg' ,null,null,0,0,'jpg' );
				}
				$fpdf->ln(15);
				$fpdf->SetLeftMargin(25);
				$fpdf->Cell(70 ,10,'' ,0);
				//$fpdf->Cell(0 ,10,$acq->getUser()->getAgency()->getName().', le '.date('d/m/Y'),0);
				$fpdf->Cell(0 ,10,$acq->getUser()->getAgency()->getName().', le '.date('d/m/Y'),0);
				$fpdf->ln(10);
				$fpdf->Cell(70 ,10,'' ,0);
			...
			foreach($mandats as $ma){
				if($initDate != $ma['dateVisite']){
					if($initDate!=  Tools::dateEnToTime('1970-09-01')){
					// Fin de la page
					$fpdf->SetFont('Times','',12);
					$fpdf->ln(8);
				...
				$initDate = $ma['dateVisite'];
			}
			// duplication du code dans l'entete du while (juste au dessus)
			$fpdf->SetFont('Times','',12);
			$fpdf->ln(8);
			$fpdf->MultiCellTag(0 ,5,utf8_decode('<b>En conséquence, nous nous engageons expressément :</b>' ) ,0);
			$fpdf->ln(4);
...
Salutation dévouées."
		...
		}if(isset($get['page'])&&$get['page']=='afficheEscalTerrain'){
			...
			if(empty($post)){
				...
				$fpdf->AddFont('MonotypeCorsiva','','mtcorsva.php');
				$fpdf->AddPage();
				$fpdf->Image(Constant::DEFAULT_URL_PICTURE_MODULE_DIRECTORY.'documents/header_escalterrain.jpg',0,0,210);
				$fpdf->Ln(50);
				...
				$fpdf->Image( Constant::DEFAULT_URL_PICTURE_MODULE_DIRECTORY.'terrain/'.$mandate->getPictureByDefault()->getName(),null,null,136,0,'jpg' );
				$fpdf->ln(8);
		...
		}if(isset($get['page'])&&$get['page']=='afficheEscalTerrainExclu'){
			$mandate = Mandate::load($this->_pdo,$get['action']);
			if(empty($post)){
				$this->_template = dirname(__FILE__).'/views/pre_affiche.tpl';
				$this->_title="Affiche exclusivité escalimmo";
				$this->_smarty->assign('ville',$mandate->getCity()->getName());
				$this->_smarty->assign('secteur',$mandate->getCity()->getSector()->getName());
				$this->_smarty->assign('corps',$mandate->getCommentaireApparent());
				// appel du stript de modification/confirmation...
				...
				$fpdf->Cell(0,10,"EXCLUSIVITE ESCAL'IMMO" ,0,1,'C',true);
		...
		if(isset($get['page'])&&$get['page']=='afficheEscalTerrainNouv'){
			$mandate = Mandate::load($this->_pdo,$get['action']);
			if(empty($post)){
				$this->_template = dirname(__FILE__).'/views/pre_affiche.tpl';
				$this->_title="Affiche exclusivité escalimmo";
				...
				$fpdf->SetTextColor(255,255,255);
				$fpdf->SetFillColor(255,0,0);
				$fpdf->Cell(0,10,"NOUVEAUTE ESCAL'TERRAIN" ,0,1,'C',true);
		...
		if(isset($get['page'])&&$get['page']=='afficheEscalTerrainVendu'){
			$mandate = Mandate::load($this->_pdo,$get['action']);
			if(empty($post)){
				$this->_template = dirname(__FILE__).'/views/pre_affiche.tpl';
				$this->_title="Affiche vendu par escalimmo";
				$this->_smarty->assign('ville',$mandate->getCity()->getName());
				$this->_smarty->assign('secteur',$mandate->getCity()->getSector()->getName());
				$this->_smarty->assign('corps',$mandate->getCommentaireApparent());
		...
		}if(isset($get['page'])&&$get['page']=='afficheEscal'){
			...
			if(isset($post['send'])){
				...
				if(empty($error)){
					...
					$fpdf->Image(Constant::DEFAULT_URL_PICTURE_MODULE_DIRECTORY.'documents/logoEscalimmo.jpg',0,0,50);
					...
					if($dejaVendu){
						...
						$fpdf->SetFillColor(255,0,0);
						$fpdf->SetY(32);
						$fpdf->Cell(0,10, utf8_decode("NOUVEAUTÉ ESCAL'IMMO") ,0,1,'C',true);
					}
					$fpdf->Output();
					exit();
				}
...
$doc->update();
		...
		if(empty($post)){
			...
			if(!empty($post) && $post['arrayPicture'][2]){
				...
				$fpdf->cell(55,25, $fpdf->Image( $logo,$fpdf->GetX()+5,$fpdf->getY()+1,50,0,'jpg') ,1);
				$fpdf->cell(5,25);
				$fpdf->cell(130,25, '',1);
				...
				$fpdf->MultiCellTag(70,20,utf8_decode(Tools::replaceSpecialCharByIso( '<h1>Prix : '.Tools::grosNombre( round($mandate->getPriceFai() ,0) ).' € FAI</h1>') ),1,'C');
